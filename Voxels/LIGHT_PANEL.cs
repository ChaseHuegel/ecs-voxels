...
namespace Swordfish
{
    public class LIGHT_PANEL : BehaviorBlock
    {
		...
		public override Coord2D getTexture(Direction _face)
		{
			...
			if (chunk.getVoxelObject() != null && isFaceCulled(_face) == false && isFaceBlocked(_face) == false)
			{
				...
				case Direction.ABOVE:
					...
					else if (neighborMatch[(int)_face, (int)Direction.SOUTH] == true) { row += 1; }
					else if (neighborMatch[(int)_face, (int)Direction.EAST] == true)
					{
						column += 1;

						if (neighborMatch[(int)_face, (int)Direction.WEST] == true) { column += 1; }
					}
					else if (neighborMatch[(int)_face, (int)Direction.WEST] == true) { column += 3; }
				...
				case Direction.SOUTH:
					...
					else if (neighborMatch[(int)_face, (int)Direction.BELOW] == true) { row += 1; }
					else if (neighborMatch[(int)_face, (int)Direction.EAST] == true)
					{
						column += 1;
				...
				case Direction.EAST:
					...
					else if (neighborMatch[(int)_face, (int)Direction.NORTH] == true)
					{
						column += 1;

						if (neighborMatch[(int)_face, (int)Direction.SOUTH] == true) { column += 1; }
					}
					else if (neighborMatch[(int)_face, (int)Direction.SOUTH] == true) { column += 3; }
		...
		public void PlaceLight(Vector3 _position, int _index)
		{
			...
			light.color = new Color(0.6f, 0.6f, 0.6f); light.range = 5.0f; light.intensity = 10.0f;
